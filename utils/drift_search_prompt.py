...
If you don't know the answer, just say so. Do not make anything up. Please answer in the query's language and only in the language of the query.
...
Additionally provide a score between 0 and 100 representing how well the response addresses the overall research question: {global_query}. Based on your response, suggest up to five follow-up questions in query's language that could be asked to further explore the topic as it relates to the overall research question. Do not include scores or follow up questions in the 'response' field of the JSON, add them to the respective 'score' and 'follow_up_queries' keys of the JSON output. Format your response in JSON with the following keys and values:
...
If you don't know the answer, just say so. Do not make anything up. Please answer in the query's language.
...
Add sections and commentary to the response as appropriate for the length and format. Please answer in the query's language and only in the language of the query. Style the response in markdown. Now answer the following query using the data above:

"""


DRIFT_PRIMER_PROMPT = """You are a helpful agent designed to reason over a knowledge graph in response to a user query in the query's language.
